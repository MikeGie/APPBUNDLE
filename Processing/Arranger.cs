...
namespace WebApplication.Processing
{
    ...
    public class Arranger
    {
        ...
        public readonly string OutputSTEP = Unique(".step");
        public readonly string OutputDWG = Unique(".dwg");
        ...
        /// <summary>
        /// Move generated STEP file to the correct place.
        /// </summary>
        internal async Task MoveStepAsync(Project project, string hash)
        {
            ...
            var ossNames = project.OssNameProvider(hash);
            await bucket.RenameObjectAsync(OutputSTEP, ossNames.Step);
        }

        internal async Task<ProcessingArgs> ForDwgAsync(string inputDocUrl, string topLevelAssembly)
        {
            var bucket = await _userResolver.GetBucketAsync();
            var dwgUrl = await bucket.CreateSignedUrlAsync(OutputDWG, ObjectAccess.Write);
        ...
        /// <summary>
        /// Move generated DWG file to the correct place.
        /// </summary>
        internal async Task MoveDwgAsync(Project project, string hash)
        {
            ...
            var ossNames = project.OssNameProvider(hash);
            await bucket.RenameObjectAsync(OutputDWG, ossNames.Dwg);
        }
        /// <summary>
        ...
        {
            var bucket = await _userResolver.GetBucketAsync();
            var stepUrl = await bucket.CreateSignedUrlAsync(OutputSTEP, ObjectAccess.Write);
            ...
            return new ProcessingArgs
            {
                ...
                TLA = topLevelAssembly,
                StepUrl = stepUrl,
                RfaUrl = stepUrl  // Add this line to make sure RfaUrl is also set
            };
        }
