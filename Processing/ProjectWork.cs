...
namespace WebApplication.Processing
{
    ...
    public class ProjectWork
    {
        ...
        /// <summary>
        /// Generate STEP file for the project.
        /// </summary>
        public async Task<(FdaStatsDTO stats, string reportUrl)> GenerateStepFileAsync(string projectName, string hash)
        {
            _logger.LogInformation($"Generating STEP file for hash {hash}");
            ...
            {
                var stats = await bucket.DeserializeAsync<Statistics[]>(ossNames.StatsRFA); // Reuse RFA stats for now
                return (FdaStatsDTO.CreditsOnly(stats), null);
            }

            // OK, nothing in cache - generate it now
            var inputDocUrl = await bucket.CreateSignedUrlAsync(ossNames.GetCurrentModel(storage.IsAssembly));
            ProcessingArgs stepData = await _arranger.ForStepAsync(inputDocUrl, storage.Metadata.TLA); // Reuse RFA processing args for now

            ProcessingResult result = await _fdaClient.GenerateStep(stepData);
            if (!result.Success)
            {
                _logger.LogError($"{result.ErrorMessage} for project {project.Name} and hash {hash}");
                throw new FdaProcessingException($"{result.ErrorMessage} for project {project.Name} and hash {hash}", result.ReportUrl);
            ...
            await bucket.UploadAsJsonAsync(ossNames.StatsRFA, result.Stats); // Reuse RFA stats for now
            return (FdaStatsDTO.All(result.Stats), result.ReportUrl);
        }
        /// <summary>
        /// Generate DWG file for the project.
        /// </summary>
        public async Task<(FdaStatsDTO stats, string reportUrl)> GenerateDwgFileAsync(string projectName, string hash)
        {
            _logger.LogInformation($"Generating DWG file for hash {hash}");
            ...
            ProcessingResult result = await _fdaClient.GenerateDwg(dwgData);
            if (!result.Success)
            {
                _logger.LogError($"{result.ErrorMessage} for project {project.Name} and hash {hash}");
                throw new FdaProcessingException($"{result.ErrorMessage} for project {project.Name} and hash {hash}", result.ReportUrl);
