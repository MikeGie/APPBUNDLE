...
namespace WebApplication.Controllers
{
    public class JobsHub : Hub
    {
        ...
        public async Task CreateStepFileJob(string projectId, string hash, string token)
        {
            _logger.LogInformation($"invoked CreateStepFileJob, connectionId : {Context.ConnectionId}");

            _profileProvider.Token = token;

            // create job and run it
            var job = new StepFileJobItem(_logger, projectId, hash, _projectWork, _linkGenerator);
            await RunJobAsync(job);
        }
